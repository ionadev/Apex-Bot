...
class Player {
	...
	async resolve(query, array) {
		const restnode = nodes[0];
		if (!/^http|(sc|yt)search/.test(query)) query = `ytsearch:${query}`;
		const isSearch = /^(sc|yt)search/.test(query);
		const { body } = await snek.get(`http://${restnode.host}:${restnode.port}/loadtracks`)
			.query({ identifier: query })
			.set('Authorization', restnode.password);
	...
	async play(player) {
		...
		const musicInterface = this.player.play(song.track);
