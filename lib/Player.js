...
class Player {
	...
	async play(player) {
		...
		if (!this.queue.songs.length) {
			return this.textChannel.send('No songs left in the music queue!').then(() => this.destroy());
		}
		this.currentSong = song;

		await this.delayer(500);

		return this.player.play(song.track).then(musicInterface => {
			musicInterface.once('end', async (data) => {
				if (data.reason === 'REPLACED') return;
				await this.textChannel.send(`Now playing: **${song.title}**, by ${song.author}, as requested by ${song.requester}!`);
