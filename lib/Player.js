	...
	async play(player) {
		...
		this.player.on('end', async () => {
			if (!this.queue.length) {
				await this.textChannel.send('The queue is empty...');
				return this.destroy();
			}
		...
		return this.queue.length ?
			embed.setDescription([
				...
				// eslint-disable-next-line
					.map((song, index) => `${((page - 1) * 10) + index + 1}: [${song.title}](${song.uri}) (${this.currentSong.isStream ? 'LIVE' : this.constructor.showSeconds(song.length / 1000)}`).join('\n')}`
			])
