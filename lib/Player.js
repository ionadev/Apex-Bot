...
class Player {
	...
	async add(user, song, cb) {
		...
		let item = await this.resolve(song, cb);
		item = Object.assign({ requester: user }, item);
	...
	async play(player) {
		...
		channel.send(`Now playing: **${song.title}**, by ${song.author}, as requested by ${song.requester}!`);
