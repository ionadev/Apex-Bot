...
module.exports = class extends Command {
  ...
  async run(message,  [args]) {
    ...
    let res;
      ...
      // raw
      let titleRaw = root.querySelector('title')      
      let imgRaw = root.querySelector('.figure')
      let descR = root.querySelectorAll('table.vitals-table')
      let descR2 = (descR[descR.length-2].childNodes[0].childNodes)
      
      // data
      let pokeName = titleRaw.childNodes[0].rawText.split(' Pokédex')[0]
      let img = imgRaw.childNodes[1].rawAttrs.split('"')[1]
      let description = descR2[descR2.length-1].childNodes[3].childNodes[0].rawText
      
      let data = root.querySelectorAll('.vitals-table')
      
      // pokedex data
      let dexNum = entities.decode(data[0].childNodes[1].childNodes[1].childNodes[3].childNodes[0].childNodes[0].rawText)
      ...
      poke.EV = EVs;
      poke.catchRate = `${catchR1}${catchR2 ? catchR2 : ""}`
      poke.happiness = `${baseH1}${baseH2 ? baseH2 : ""}`
